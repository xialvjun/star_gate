...
export default class ZeroFrame {
  ...
  onRequest(cmd, message) {
    const listeners = this.on_request_listeners[cmd];
    if (listeners && listeners.length > 0) {
      return listeners.forEach(l => l(message));
    }
    this.log("Unknown request", message);
  }

  on_cmd(cmd, callback) {
    this.on_request_listeners[cmd] = this.on_request_listeners[cmd] || [];
    this.on_request_listeners[cmd].push(callback);
    return () => this.on_request_listeners[cmd] = this.on_request_listeners[cmd].filter(c => c !== callback);
  }
